...
def book():
    ...
    if request.method == "POST":
        ...
        if not all([doctor_id, appt_date, time_slot]):
            ...
            result = book_appointment(int(doctor_id), patient["id"], appt_date, time_slot)
            messages = {
...
def cancel(appointment_id):
    patient = get_patient_by_user(session["user_id"])
    # Pass patient_id so cancel_appointment verifies ownership
...
@appointment_bp.route("/confirm-waiting/<int:waiting_id>", methods=["POST"])
@doctor_required
...
@appointment_bp.route("/api/slots/<int:doctor_id>")
@login_required
